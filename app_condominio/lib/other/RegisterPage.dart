...
class _RegisterPageState extends State<RegisterPage>
{
  ...
  Widget build(BuildContext context) 
  {
    return ListView
      (
        ...
        children: <Widget>[
          SizedBox(height: 10,),
          Center( child: Text("Ingrese datos del co-propietario", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 22,), ), ),
          SizedBox(height: 10,),
          TextField( 
          ...
          TextField( 
              ...
              Container(
                ...
                child: Row(
                  children: <Widget>[
                    // SizedBox(width: 10.0 ,),
                    Text("Seleccione: ", style: TextStyle(fontWeight: FontWeight.normal, fontSize: 16,), ),
                    DropdownButton( value: torre, items: _getItems(), onChanged: _selectTorre, ),
                    SizedBox(width: 10.0 ,),
              ...
              SwitchListTile(  activeColor: Colors.blue,
                // isThreeLine: true,
                dense: true, title: Text('Tiene ud. mascota'), value: mascota, onChanged: _changedM, ),
              SizedBox(width: 10.0 ,),
              _crearBoton(),  
              Text(ingresados?"Datos Ingresados correctamente.":"", style: TextStyle(fontWeight: FontWeight.normal, fontSize: 16, color: Colors.red), ),
        ],/*),
      ),*/
    );
  }
  ...
  List<DropdownMenuItem<String>>_getItems()
  {
    ...
    list.add( DropdownMenuItem(child: Text('Torre A'), value: 'A',));
    list.add( DropdownMenuItem(child: Text('Torre B'), value: 'B',));
    list.add( DropdownMenuItem(child: Text('Torre C'), value: 'C',));
    return list;
  }
...
_evaluateMail( String text )
{
  ...
  void _registrar() async
  {
    ...
    DaoNeighbour dao = new DaoNeighbour( imei: widget.imei,   
                  name: this.nombre, 
                  mail: this.mail,
                  depto: this.depto, 
                  pet: this.mascota, 
    ...
    try{
      ...
      if( resp.statusCode == 201 ) //201 es CREADO
      {
        ...
        DataUser.getInstance().setDao(new DaoQuestion( nombre: this.nombre, depto: this.depto, torre: this.torre));
